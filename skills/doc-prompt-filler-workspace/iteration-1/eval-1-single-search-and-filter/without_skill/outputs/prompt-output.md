...
為 CYBERBIZ 後台 **/admin/orders_v2** 撰寫一份**末端商家**使用的教學文件。
...
1. **功能介紹** — 這個頁面是什麼、用途 { #intro-search-and-filter }
2. **頁面功能總覽**(若有,以表格呈現) { #overview-search-and-filter }
3. **計費或規則說明**(若有) { #pricing-search-and-filter }
4. **使用前提與限制**(若有) { #prerequisites-search-and-filter }
5. **操作步驟** — Step-by-step,依**使用情境**分段 { #operate-search-and-filter }
6. **重要規範與限制**(若有) { #specs-search-and-filter }
7. **後續操作** — 完成主要操作後商家可能接著做的事 { #next-steps-search-and-filter }
8. **常見問題(FAQ)** { #faq-search-and-filter }
9. **參考資料** { #reference-search-and-filter }
...
* **主文**:`docs/orders/search-and-filter.md`(例如 `docs/orders/search-and-filter.md`)
* **對照表**:`docs/orders/references/search-and-filter-columns.md`、`docs/orders/references/search-and-filter-statuses.md` 等,**每張對照表獨立一個檔案**
...
6. ☐ 列表項目皆為單一物理行(§C-1)
7. ☐ 對照表已拆檔到 `docs/orders/references/`,主文用 `data-preview` 跳轉
8. ☐ 新舊版差異(若適用)已用 `=== "..."` 分頁呈現
...
* 若新增了對照表,提供對照表的目標檔案路徑(`docs/orders/references/{{file}}.md`)
...
文件 A:
  ...
  頁面: /admin/orders_v2
  主題: 訂單搜尋與篩選
  檔案路徑: docs/orders/search-and-filter.md
```
