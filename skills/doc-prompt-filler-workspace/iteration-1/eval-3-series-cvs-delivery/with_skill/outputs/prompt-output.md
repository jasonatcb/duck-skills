...
為 CYBERBIZ 後台 **/admin/logistics/cvs、/admin/orders_v2 與配送狀態查詢頁面** 撰寫一份**末端商家**使用的教學文件。

主題:**超商取貨設定、超商出貨流程與配送狀態對照**
...
* **情境**:`series`
* **關聯文件**(series 專用):超商取貨設定 → 超商出貨流程 → 配送狀態對照表

#### 各文件變數

```yaml
文件 A:
  category: cvs-pickup-setting
  module: payments-and-logistics
  頁面: /admin/logistics/cvs
  主題: 設定超商取貨
  檔案路徑: docs/payments-and-logistics/cvs-pickup-setting.md

文件 B:
  category: cvs-shipping
  module: payments-and-logistics
  頁面: /admin/orders_v2
  主題: 超商出貨流程
  檔案路徑: docs/payments-and-logistics/cvs-shipping.md
...
文件 C:
  ...
  頁面: 配送狀態查詢頁面
  主題: 配送狀態對照表
  檔案路徑: docs/payments-and-logistics/delivery-status.md
```
...
**使用說明：** 這是給 Firebase CYBERBIZ 後端專案的 **Claude Code** 使用的 doc-generator prompt。複製上述 code block 中的完整內容，到該專案中貼上執行即可開始研究程式碼並產出三份系列文件（超商取貨設定 → 超商出貨流程 → 配送狀態對照表）。
